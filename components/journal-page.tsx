        ...
        <DialogContent className="sm:max-w-[400px]">
          <DialogHeader>
            <DialogTitle className="flex items-center space-x-2 text-center">
              ...
              <span className="text-lg">Payment Receipt</span>
            </DialogTitle>
          </DialogHeader>
          ...
          {lastPayment && (
            <div className="space-y-4 py-2">
              ...
              <div className="text-center border-b pb-3">
                ...
                <p className="text-xs text-gray-600">Receipt #{lastPayment.payment.id}</p>
                <p className="text-xs text-gray-600">{lastPayment.payment.date}</p>
              </div>
              ...
              <div className="space-y-3">
                <div className="grid grid-cols-2 gap-3 text-sm">
                  <div>
                    <p className="text-xs font-medium text-gray-600 uppercase">From</p>
                    <p className="font-semibold text-gray-900">{lastPayment.invoice.clientName}</p>
                    <p className="text-xs text-gray-600">{lastPayment.invoice.clientCompany}</p>
                  </div>
                  <div>
                    <p className="text-xs font-medium text-gray-600 uppercase">Invoice</p>
                    <p className="font-semibold text-gray-900">#{lastPayment.invoice.number}</p>
                    <p className="text-xs text-gray-600">{lastPayment.invoice.createdDate}</p>
                  </div>
                </div>

                {/* Payment Amount - Highlighted */}
                <div className="bg-green-50 p-3 rounded-lg text-center">
                  <p className="text-xs font-medium text-gray-600 uppercase">Payment Amount</p>
                  <p className="text-xl font-bold text-green-600">
                    {formatCurrency(lastPayment.payment.amount, lastPayment.invoice.currency)}
                  </p>
                  <p className="text-xs text-gray-600 capitalize">via {lastPayment.payment.method.replace("_", " ")}</p>
                </div>
                  ...
                  <div className="grid grid-cols-3 gap-2 text-xs text-center">
                    <div>
                      ...
                      <p className="font-semibold">
                        {formatCurrency(lastPayment.invoice.totalAmount, lastPayment.invoice.currency)}
                      </p>
                    </div>
                    <div>
                      <p className="text-gray-600">Paid</p>
                      <p className="font-semibold text-green-600">
                        {formatCurrency(lastPayment.invoice.paidAmount, lastPayment.invoice.currency)}
                      </p>
                    </div>
                    <div>
                      ...
                      <p className="font-semibold text-orange-600">
                        {formatCurrency(
                          lastPayment.invoice.totalAmount - lastPayment.invoice.paidAmount,
                          lastPayment.invoice.currency,
                        )}
                      </p>
                    </div>
                  </div>
                  <div className="flex justify-center mt-2">
                    <Badge className={`${getStatusColor(lastPayment.invoice.status)} text-xs`}>
                      {getStatusLabel(lastPayment.invoice.status)}
                    </Badge>
                  </div>
                </div>
                ...
                <div className="text-center text-xs text-gray-500 border-t pt-2">
                  ...
                  <p className="font-medium">CLIENT FACTURATION</p>
                </div>
              </div>

              {/* Action Buttons */}
              <div className="flex justify-center space-x-3 pt-3 border-t">
                <Button variant="outline" size="sm" onClick={() => setShowReceipt(false)} className="flex-1">
                  Cancel
                </Button>
                <Button
                  size="sm"
                  ...
                  onClick={() => {
                    // For now, just show an alert - you can implement actual printing later
                    alert("Print functionality will be implemented soon!")
                    // Future: window.print() or generate PDF
                  }}
                >
